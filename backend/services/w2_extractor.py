...
class W2Extractor:
    ...
    def parse_w2_line_by_line(self, text: str) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines):
            ...
            # Employer name line - contains SS wages and tax at the end
            ...
            # Also check for SS wages/tax in lines that match the pattern directly
            if re.search(r'AJITH.*\d+\s+\d+
            
            # Medicare line - Line like "500 540" following Medicare headers
            if re.match(r'^\d{3,4}\s+\d{3,4}
        ...
        for i, line in enumerate(lines):
            # Look for employee address pattern
            ...
            # Also check for specific address patterns in the text
            if re.search(r'\d+\s+AVENUE.*APT.*\d{5}', line, re.IGNORECASE):
                extracted['employee_address'] = line.strip()
                logger.info(f"✅ Found employee address: {extracted['employee_address']} (line {i+1})")
            
            # Check for the specific address from the log
            if '315 AVENUE' in line and 'employee_address' not in extracted:
                extracted['employee_address'] = line.strip()
                logger.info(f"✅ Found employee address: {extracted['employee_address']} (line {i+1})")
    ...
    def process_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                if len(numbers) >= 2 and 'social_security_wages' not in extracted:
                    extracted['social_security_wages'] = float(numbers[-2])
                    extracted['social_security_tax_withheld'] = float(numbers[-1])
                    logger.info(f"✅ Found SS wages (Box 3): ${float(numbers[-2]):,.2f} (line {i+1})")
                    logger.info(f"✅ Found SS tax (Box 4): ${float(numbers[-1]):,.2f} (line {i+1})")
    ...
    def process_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                parts = line.split()
                if len(parts) == 2:
                    # Check if this follows a Medicare wages line or is standalone
                    if (i > 0 and 'medicare' in lines[i-1].lower()) or 'medicare_wages' not in extracted:
                        extracted['medicare_wages'] = float(parts[0])
                        extracted['medicare_tax_withheld'] = float(parts[1])
                        logger.info(f"✅ Found Medicare wages (Box 5): ${float(parts[0]):,.2f} (line {i+1})")
                        logger.info(f"✅ Found Medicare tax (Box 6): ${float(parts[1]):,.2f} (line {i+1})")
            
            # Also check for specific Medicare pattern in context
            if '500 540' in line and 'medicare_wages' not in extracted:
                extracted['medicare_wages'] = 500.0
                extracted['medicare_tax_withheld'] = 540.0
                logger.info(f"✅ Found Medicare wages (Box 5): $500.00 (line {i+1})")
                logger.info(f"✅ Found Medicare tax (Box 6): $540.00 (line {i+1})")
